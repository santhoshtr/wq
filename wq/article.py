...
class Article:
    ...
    @property
    def page_url(self):
        return f"https://{self.language.lower()}.wikipedia.org/wiki/{self.title}"
    ...
    def get_page_metadata(self):
        api_url = f"https://{self.language}.wikipedia.org/api/rest_v1/page/summary/{self.title}?redirect=true"
        self.metadata: list[dict] = requests.get(api_url, timeout=100).json()
    ...
    def get_page_sections(self, max_paragraphs: int = -1) -> str:
        ...
        """
        if not self.content:
            api_url = f"https://{self.language}.wikipedia.org/api/rest_v1/page/html/{self.title}?redirect=true"
            page = requests.get(api_url, timeout=100)
